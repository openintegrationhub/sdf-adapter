...
async function getDomains(){
  ...
  const requestOptions = {
      ...
      json: true,
      headers: {
          "Authorization" : " Bearer h7aLJfyamLbNJsFmOkktL1dafPBdgxAmESxOgFRrOvatU_Ar5iFoNa3Nd5iaOID2fOPBvmT-ZSRkMkn7ZteN79TdADxzNGOwjob7DNjOBuyYRuCzOms0jdvVyBs9YQPXfR9a813m_vhK80ZetTXDMT5krI8wW_tEiuyyrNmyn_E", 
          }
  };
  ...
  domains.data.forEach(element => {
      domainNames[element.id] = element.name;
...
async function getModels(cfg){
  ...
  models.data.forEach(element => {
      modelNames[element.id] = element.name;
